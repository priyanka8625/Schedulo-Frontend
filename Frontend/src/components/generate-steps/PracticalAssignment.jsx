...
export default function PracticalAssignment({ data, onDataChange }) {
  ...
  const loadSubjects = () => {
    const data = storage.subjects.getByYearAndType(year, ['Practical', 'Both']);
    setSubjects(data);
  };
  ...
  const loadAssignments = () => {
    const data = storage.facultyAssignments.getByYearAndType(year, 'practical');
    setAssignments(data);
  };
    ...
    <div className="space-y-8">
      <div>
        <h3 className="text-2xl font-bold text-slate-800 mb-2">
          Step 3: Faculty Practical Assignment
        </h3>
        <p className="text-slate-600">
          Assign faculties to practical batches for each year
        </p>
      </div>
      ...
      <div className="bg-gradient-to-br from-orange-50 to-slate-50 rounded-xl shadow-lg p-6 border border-slate-200">
        <h4 className="text-xl font-bold text-slate-800 mb-4">
          Practical Assignments for {year} ({assignments.length})
        </h4>
        ...
        {assignments.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 grid grid-cols-2 gap-x-6 gap-y-3">
                    ...
                    <div>
                      <span className="text-xs font-semibold text-slate-500 uppercase">Division & Batch</span>
                      <p className="text-slate-800 font-semibold mt-1">
                        Division {assignment.division} - {assignment.batch}
                      </p>
                    </div>
                    <div>
          ...
          <div className="text-center py-12">
            <p className="text-slate-500 text-lg">No practical assignments for {year} yet</p>
